...
async function copyImageWindows(imagePath: string): Promise<void> {
  // Use Python ctypes script to copy image to clipboard
  // PowerShell clipboard operations hang in non-interactive terminals
  const scriptDir = path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1'));
  const pythonScript = path.join(scriptDir, 'copy_image_ctypes.py');

  // Check if Python script exists
  if (!fs.existsSync(pythonScript)) {
    throw new Error(`Python clipboard script not found: ${pythonScript}`);
  }

  await runCommand('python', [pythonScript, imagePath]);
}
...
async function copyHtmlWindows(htmlFilePath: string): Promise<void> {
  ...
  // Wrap in block to pass parameters
  const ps = `& { ${psBody} }`;
