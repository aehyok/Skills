...
function convertMarkdownToHtml(markdown: string, imageCallback: (src: string, alt: string) => string): { html: string; totalBlocks: number } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Image - Standard Markdown format: ![alt](path)
    ...
    // Image - Obsidian Wiki link format: ![[filename]] or ![[filename|alt]]
    const obsidianImgMatch = line.match(/^!\[\[([^\]|]+)(?:\|([^\]]*))?\]\]\s*$/);
    if (obsidianImgMatch) {
      flushList();
      const filePath = obsidianImgMatch[1]!;
      const alt = obsidianImgMatch[2] ?? '';
      const placeholder = imageCallback(filePath, alt);
      blocks.push(`<p>${placeholder}</p>`);
      continue;
    }
